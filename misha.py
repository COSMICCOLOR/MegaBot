    ...
    if message.text.lower() == '/addadmin' :
        ...
        if user_id not in row :  # проверяем id на пригодность
            ...
            bot.send_message(message.chat.id,
                             '''Добро пожаловать в админ-панель "FoodBot". Здесь Вы можете изменить меню и просмотреть отзывы ресторана "Літвіны"''',
...
# END code serezha + Dima_______________________________________________________________________________________________
    ...
    if call.data.split(':')[1] == "addadmin":
        ...
        add_inline_keyb.add(InlineKeyboardButton("Добавить данные о новом администраторе", callback_data="addm:adminname"))
    ...
    if call.data.split(":")[1] == "adminname" :
        bot.send_message(call.message.chat.id, "Введите имя нового администратора", reply_markup=telebot.types.ForceReply())
...
def handler_admin_first_answer(message):
    ...
    admin_first_name = message.text
    last_name, phone, position = '', '', ''
    add_adm = user_id
    with conn:
        conn.execute(admin_table, [add_adm, admin_first_name, last_name, phone, position])
    conn.commit()
    bot.send_message(message.chat.id, "Введите фамилию нового администратора")
...
def handler_admin_last_answer(message):
    ...
    with conn:
        conn.execute(f"UPDATE BotAdmins SET last_name = ? WHERE telegram_id = ?", (last_name, user_id))
    conn.commit()
...
def handler_admin_last_answer(message):
    phone = message.text
    with conn:
        conn.execute(f"UPDATE BotAdmins SET phone_number = ? WHERE telegram_id = ?", (phone, user_id))
    conn.commit()
    bot.send_message(message.chat.id, 'Отлично,введите должность!', reply_markup=telebot.types.ForceReply())

@bot.message_handler(func=lambda message: message.reply_to_message and message.reply_to_message.text in ['Отлично,введите должность!'])
def handler_admin_last_answer(message):
    position = message.text
    with conn:
        conn.executeconn.execute(f"UPDATE BotAdmins SET position = ? WHERE telegram_id = ?", (position, user_id))
    conn.commit()
