...
#START________________________________code serezha
    ...
    if call.data.split(':')[1] == 'buy':
        ...
        order_after_cart_markup.add(InlineKeyboardButton('Оставить комментарий', callback_data='1:Оставить комментарий'))
        ...
        for i in info:
            for j in i:
                total_price += j[4]
                result += f'Блюдо: {j[1]}\n'
        bot.send_message(call.message.chat.id, f'{result} Цена:{total_price}', reply_markup=order_after_cart_markup)
